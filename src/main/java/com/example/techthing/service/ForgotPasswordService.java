...
public class ForgotPasswordService {
    ...
    public void sendOTP(String mail) {
        ...
        MailBody mailBody = MailBody.builder()
                .to(mail)
                .text("This is the OTP for verify Forgot Password: " + otp)
                .subject("FORM TECHTHING WITH LUV <3")
                .build();
    ...
    public String verifyOTP(Integer otp, String mail) {
        User user = userRepository.findByMail(mail).orElseThrow(() -> new MyException(ErrorCode.USER_NOT_EXISTED));
    ...
    public ChangePasswordResponse changePassword(ChangePasswordRequest changePasswordRequest) {
        User user = userRepository.findByMail(changePasswordRequest.getMail())
                .orElseThrow(() -> new MyException(ErrorCode.USER_NOT_EXISTED));
        ForgotPassword forgotPassword = forgotPasswordRepository.findByOtpAndUser(changePasswordRequest.getOtp(), user)
                .orElseThrow(() -> new MyException(ErrorCode.INVALID_OTP));
